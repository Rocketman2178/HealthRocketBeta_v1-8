...
import type { CosmoState, CosmoAction } from '../types/cosmo';
...
export function CosmoProvider({ children }: { children: React.ReactNode }) {
  ...
  // Auto-show Cosmo on first visit or new day
  useEffect(() => {
    if (!user) return;
    ...
    // Always show on page refresh unless dismissed
    dispatch({ type: 'SHOW_COSMO' });
  }, [user, state.disabledUntil]);
